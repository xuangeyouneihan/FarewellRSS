...
| 变量                                     | 默认值              | 说明                                                                                                         |
| ---------------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------ |
| `FAREWELL_RSS_PASSWORD_MAX_CONCURRENCY` | `min(4, CPU 核数)` | 计算 bcrypt 密码哈希 / 校验的线程池大小。密码运算很贵（单次约 0.2s）且同步阻塞，跑在专属线程池里以免卡住事件循环。bcrypt 会释放 GIL，所以池内能真并行（实测 4 并发约 4 倍加速）。 |

- 这是**专门**给密码运算用的池，不与订阅源抓取等其他任务共用（否则抓取会把池占满，登录请求只能排队）。
- 调小 → 密码运算并发更低，相当于给暴力破解加了一层限流；调大 → 高并发登录时排队更少，吞吐可一直提升到约 CPU 核数为止（线程超过核数只会互相抢 CPU，没有额外收益）。
- 必须是不小于 1 的整数，否则启动时报错（该变量在进程启动时读取，改后需重启）。
